...
Use bead IDs as your threading anchor, but do not use Agent Mail or `br` as
the Gas City claim source.

```
1. Claim work:       gc hook --claim --json → GC_BEAD_ID=bd-123
...
5. Complete:         bd update/close the claimed bead with required metadata,
                     release_file_reservations(...), final message
```

`br ready`, `br claim`, broad `bd ready`, and mail-thread assignment are
coordination hints at most; they are not valid live claim mechanisms inside
Gas City workers.
