...
## Gas City Adapter Rule

Inside Gas City role workers, `br` examples are tracker maintenance examples,
not live claim instructions. Do not use `br ready`, `br claim`, or broad bead
searches to select work. Workers claim only the bead returned by
`gc hook --claim --json`; after that, use explicit `bd show/update/close` on
the claimed bead id with the metadata required by the bead contract.
