...
`bv` ranks and explains the graph; it does not assign live work in Gas City.
Use `bv` to choose what to route next, then dispatch through Gas City metadata
and have workers claim via `gc hook --claim --json`.
