...
Use this skill to design, create, and polish the bead graph. Do not use its
upstream `br ready` examples as worker claim instructions inside Gas City.
Gas City workers receive routed work through `gc hook --claim --json`; `bv` and
`br`/`bd` analysis are planning and maintenance inputs, not a substitute claim
source.
