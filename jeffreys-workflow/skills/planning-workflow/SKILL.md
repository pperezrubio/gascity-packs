...
- It has been **converted to beads** with the dependency graph intact, so
  Gas City can route ready work and implementation agents can claim it via
  `gc hook --claim --json` without re-reading the plan.
